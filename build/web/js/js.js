...
//    }
    ...
    $("#newConfigModal").click(function () {
        $("#myModalOp").modal({backdrop: "static", keyboard: "false"});
    });

    $("#saveConfigModal").click(function () {
        guardarConfig();
...
function cargarConfig() {
    var indexAux = $("#selOpciones option:selected").index();
...
function guardarConfig() {
    if (comprobarConfig()) {
        ...
        $.ajax({
            method: "POST",
            url: url,
            data: {nombre: name, dificultad: ahoraDif, nave: naveImg, lugar: lugarAterrizaje},
            success: function (u) {
                arrayConfig.push([name, ahoraDif, naveImg, lugarAterrizaje]);
                $("#selOpciones").append("<option id=" + indexConfig + ">" + name + " (" + ahoraDif +
                        ", " + naveImg + ", " + lugarAterrizaje + ")</option>");
                indexConfig++;
...
function comprobarConfig() {
    if ($("#nameConfig").val().length <= 0) {
        alert("Por favor, introduce un nombre para la configuración");
        return false;
    }
    for (var i = 0; i < arrayConfig.length; i++) {
        if ($("#nameConfig").val() == arrayConfig[i][0]) {
            alert("Ese nombre de configuración ya está elegido, por favor, elige otro");
            return false;
        }
    }
    return true;
